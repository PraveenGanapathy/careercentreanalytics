...
function updateMetricCards(data) {
    ...
    categories.forEach(category => {
        ...
        const currentYearData = categoryData.filter(d => d.fiscal_year === selectedYear);
        const previousYearData = categoryData.filter(d => d.fiscal_year === previousYear);
        
        // Calculate total values for current and previous year
        ...
        const maxPossible = categoryData.length * 100; // Assuming each metric can score up to 100
        const score = Math.round((currentTotal / maxPossible) * 100);
...
// Show metric details in a modal
